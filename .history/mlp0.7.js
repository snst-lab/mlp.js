...
melonpan.For = class {
    ...
    static expand(self, data){
        ...
        data.forEach(function(element){
            var fragment = '';
            if(element.src instanceof Array){
                element.src.forEach(function(src){
                    fragment += master.replace(new RegExp( '#'+element.dst, 'g'), src).unescapeHTML();
                });
            }else{
                fragment += master.replace(new RegExp( '#'+element.dst, 'g'), element.src).unescapeHTML();
            }
            master = fragment;
        });
        master = melonpan.For.readScript(master);
        $(self).after($(master));
        $(self).hide();
    }
    ...
    static readScript(text){
        if( text.match('{{') && text.match('}}')){
            return  melonpan.For.parseScript(text);
        }else{
            return text;
        }
    }
...
melonpan.ForAfterWait = class {
    constructor(self,wait){
        ...
        if(attr!==null){
            const data = melonpan.For.parse(attr);
            melonpan.ForAfterWait.expand(self, data, wait);
        }
    }
    ...
    static expand(self, data, wait){
        ...
        // self.parentNode.insertBefore(wrapper, self.nextSibling);
        // wrapper.outerHTML= master;
        // wrapper.setAttribute('afterwait',wait);
        // self.style.display = 'none';
        // self.outerHTML = master;
        
        $(self).after($(master).attr('afterwait',wait));
        $(self).hide();
    }
}
...
melonpan.Main = class {
    ...
    static async init(){
        ...
        await melonpan.Main.preLoader(melonpan.loading);
                ...
                if(wait!==null){
                    if(Class == melonpan.For){
                        ...
                        $(document).on(wait,function(){
                            $(self).show();
                            new melonpan.ForAfterWait(self,wait);
                        });
    ...
    static readSetting(){
        ...
        if(typeof setting['loading']!=='undefined'){
            melonpan.loading = setting['loading'];
        }
    }
    ...
    static preLoader(preloader){
        if(preloader.match('.html')){
            const xhr = new XMLHttpRequest();
            xhr.open("GET", preloader , false); //true : async
            xhr.send(null);
            if (xhr.status == '200') {
                const loading = document.createElement('div');
                loading.id = "_loading";
                loading.style = "position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:101;background:white;";
                loading.innerHTML = xhr.responseText;
                document.querySelector('body').appendChild(loading);
            } else {
               console.log(preloader + ' is not found.');
            }
        }
        else if(preloader.match('.png')){
            const loading = document.createElement('div');
            loading.id = "_loading";
            loading.style = "position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:101;background:white;";
        ...
        setTimeout(function(){$("#_loading").fadeOut('slow');},1000);
    }
}
